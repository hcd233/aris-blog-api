...
func ListChildrenCategoriesHandler(c *gin.Context) {
	...
	parentCategory, err := categoryDAO.GetByID(db, uri.CategoryID, []string{"id", "name", "parent_id", "user_id"}, []string{})
	if err != nil {
		c.JSON(http.StatusBadRequest, protocol.Response{
	...
	categories, pageInfo, err := categoryDAO.PaginateChildren(db, parentCategory, []string{"id", "name", "parent_id"}, []string{}, param.Page, param.PageSize)
	if err != nil {
		c.JSON(http.StatusBadRequest, protocol.Response{
